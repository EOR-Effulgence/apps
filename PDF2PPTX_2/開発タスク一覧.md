...
### 🔴 最優先（今すぐ着手）
1. **TASK-006**: PDF→PNG変換コア実装 (`src/core/pdf_processor.py`)
2. **TASK-007**: PowerPoint生成コア実装 (`src/core/pptx_generator.py`)
3. **TASK-008**: ファイルユーティリティ実装 (`src/utils/file_utils.py`)

### 🟡 高優先（コア完成後）
4. **TASK-011**: メインウィンドウフレームワーク (`src/gui/main_window.py`)
5. **TASK-013**: ファイルドロップゾーン実装
6. **TASK-014**: 設定パネル実装
...
|-------|------|-----------|------|------|
| Phase 1 | Week 1 | プロジェクト基盤、設定システム | 🔴 必須 | **60%** ✅ |
| Phase 2 | Week 2-3 | PDF/PPTX処理エンジン | 🔴 必須 | **0%** 📝 |
| Phase 3 | Week 3-4 | GUI実装 | 🔴 必須 | **0%** 📝 |
| Phase 4 | Week 4-5 | システム統合、Windows統合 | 🔴 必須 | **0%** 📝 |
| Phase 5 | Week 5-6 | テスト・品質保証 | 🔴 必須 | **0%** 📝 |
...
### 🔴 必須タスク（MVP用）
- ✅ 完了: 1, 2, 4, 5
- 📝 残り: 6-7, 9, 11, 13-14, 16-19, 24, 26, 29-30
